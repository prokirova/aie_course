...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # 1. Проверка на постоянные колонки (все значения одинаковые)
    has_constant = any(col.unique == 1 and col.non_null > 0 for col in summary.columns)
    ...
    #  Расчёт quality_score с учётом новых флагов
    score = 1.0
    ...
    if flags["has_high_cardinality_categoricals"]:
        score -= 0.05  # менее критично, но всё же подозрительно
