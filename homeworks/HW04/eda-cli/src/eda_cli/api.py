...
@app.post(
    ...
    tags=["quality"],
    summary="Получить все флаги качества данных из CSV",
    description=(
        "Принимает CSV-файл, анализирует его с помощью EDA-ядра (summarize_dataset, missing_table, compute_quality_flags) "
        "и возвращает полный набор булевых флагов качества, включая: "
        "too_few_rows, too_many_columns, too_many_missing, has_constant_columns, has_high_cardinality_categoricals и др."
    ),
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> dict[str, dict[str, bool]]:
    # НЕ вызываем file.read() — работаем напрямую с file.file
    try:
        # Возвращаем указатель в начало (на всякий случай)
        await file.seek(0)
        df = pd.read_csv(file.file)
    ...
    if df.empty:
        raise HTTPException(status_code=400, detail="CSV is empty after parsing.")
    ...
    if not flags_bool:
        raise HTTPException(status_code=500, detail="No boolean quality flags were generated.")
