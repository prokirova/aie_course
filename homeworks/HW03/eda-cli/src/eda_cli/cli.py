...
def report(
        ...
        min_missing_share: float = typer.Option(0.1,
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        if not missing_df.empty:
            ...
            if not problematic_cols.empty:
                f.write("## ⚠️ Проблемные колонки (высокая доля пропусков)\n\n")
                f.write("| Колонка | Пропусков | Доля |\n")
                f.write("|---------|-----------|------|\n")
                for col, row in problematic_cols.iterrows():
                    f.write(f"| `{col}` | {int(row['missing_count'])} | {row['missing_share']:.2%} |\n")
                f.write("\n")
        ...
        if not top_cats:
            ...
            f.write(f"Показано top-{top_k_categories} значений для каждой категориальной колонки.\n")
            f.write("См. файлы в папке `top_categories/`.\n\n")

        f.write("## Гистограммы числовых колонок\n\n")
        f.write(f"Показано максимум {max_hist_columns} числовых колонок.\n")
